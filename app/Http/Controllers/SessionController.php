...
class SessionController extends Controller
{
    public function HomePage(Request $request){
        ...
        if(Auth::user()->role == 'user'){
            ...
            // $netOverdue = Invoice::where('user_id',$id)->where('status','=','OVERDUE')->where('is_deleted','=',0)->sum('due_amount');
            //$netAmount = DB::table("invoices")->where('user_id',$id)->where('is_deleted','=',0)->sum('net_amount');
            // $dueAmount = DB::table("invoices")->where('user_id',$id)->where('is_deleted','=',0)->sum('due_amount');
            $sentAmount = DB::table("invoices")->where('user_id',$id)->where('status','=','SENT')->where('is_deleted','=',0)->sum('net_amount'); 
            //dd($sentAmount);
            // $paidAmount = DB::table("invoices")->where('user_id',$id)->where('status','=','PAID-STRIPE')->sum('net_amount');
            // $depositAmount = DB::table("invoices")->where('user_id',$id)->where('status','=','DEPOSIT_PAID')->orWhere(function($q){
            //     $q->where('status','=','OVERDUE')->where('net_amount','!=','due_amount');
            // })->sum('deposit_amount'); //dd($depositAmount);

            $requestYear = $request->revenu;
           if($requestYear)
            { 
              $yearData = $request->revenu; 
            }else{
              $yearData = array(date('Y'));
            }

            $requestRevenuYrs = $request->client_revenu;
            if($requestRevenuYrs){
              $clientRevenuYrs = $request->client_revenu;
            }else{
              $clientRevenuYrs = array(date('Y'));
            }
            ///start total revenu amount data current years 
            $sentInvoiceAmount = DB::table("invoices")->where('user_id',$id)->where('status','=','SENT')->where('is_deleted','=',0)->whereRaw('year(`created_at`) = ?', $yearData)->sum('net_amount');
            $totalPaidAmount = DB::table("invoices")->where('user_id',$id)->where('status','=','PAID-STRIPE')->where('is_deleted','=',0)->whereRaw('year(`created_at`) = ?', $yearData)->sum('net_amount'); //dd($totalPaidAmount);
            $totalOfflineAmount = DB::table("invoices")->where('user_id',$id)->where('status','=','PAID-OFFLINE')->where('is_deleted','=',0)->whereRaw('year(`created_at`) = ?', $yearData)->sum('net_amount'); //dd($totalOfflineAmount);
            $totalBankWireAmount = DB::table("invoices")->where('user_id',$id)->where('status','=','PAID-BANKWIRE')->where('is_deleted','=',0)->whereRaw('year(`created_at`) = ?', $yearData)->sum('net_amount'); //dd($totalBankWireAmount);
            $netOverdueAmount = Invoice::where('user_id',$id)->where('status','=','OVERDUE')->where('is_deleted','=',0)->whereRaw('year(`created_at`) = ?', $yearData)->sum('net_amount');
            $onlyInvoiceDepositAmount = DB::table("invoices")->where('user_id',$id)->where('status','=','DEPOSIT_PAID')->where('is_deleted','=',0)->whereRaw('year(`created_at`) = ?', $yearData)->sum('deposit_amount'); 
            ///end total revenu amount data

            $revenueAmount = DB::table("invoices")->where('user_id',$id)->where('status','!=','DRAFT')->where('is_deleted','=',0)->sum('net_amount');
            ...
            $overdueInDepositAmount = DB::table("invoices")->where('user_id',$id)->where('status','=','OVERDUE')->where('net_amount','!=','due_amount')->where('is_deleted','=',0)->sum('deposit_amount'); 
              ...
              $invoices = Invoice::where('user_id', $id)->where('is_deleted','=',0)->latest()->paginate(10);
              //dd($invoices);


             $clients = Clients::with('user')->where('user_id', $id)->latest()->get(); //dd($clients);
            foreach ($clients as $key => $client) {
              $totalInvoiceAmount = DB::table("invoices")->where('client_id', $client->id)->where('status','!=','DRAFT')->where('is_deleted','=',0)->whereRaw('year(`created_at`) = ?', $clientRevenuYrs)->sum('net_amount');
              $companyImage = UserCompany::where('id', $client->companies_id)->first();
              $client->invoiceRaised = $totalInvoiceAmount;
              $client->image = $companyImage->logo;
            }

            // $clientComapny = UserCompany::with('user')->where('user_id', $id)->latest()->get(); //dd($clientComapny);
            //   foreach ($clientComapny as $key => $company) {
            //       $totalInvoiceAmount = DB::table("invoices")->where('companies_id', $company->id)->where('status','!=','DRAFT')->where('is_deleted','=',0)->whereRaw('year(`created_at`) = ?', $clientRevenuYrs)->sum('net_amount');
            //       $company->invoiceRaised = $totalInvoiceAmount;
            //       $company->halfname = explode(" ", $company->name);
            //   }             
           ...
           /// invoice net amount total where status Paid
           // select * from table where YEAR(date) = YEAR(CURDATE())
           //WHERE YEAR(created_at) = YEAR(CURDATE()- INTERVAL 1 YEAR)
           
           $paids =DB::table('invoices')
                     ->select(DB::raw('sum(net_amount) as amount'),DB::raw('DATE_FORMAT(created_at, "%Y-%m-01") as month'))
                     ->groupBy('month')
                     ->orderBy('month','asc')
                     ->whereIn('status',['PAID-STRIPE','PAID-OFFLINE','PAID-BANKWIRE'])
                     ->where('is_deleted','=',0)
                     ->where('user_id', '=', $id)
                  ...
                  foreach($paids as $paid){
                          $barChartData[$paid->month]['paid'] = $paid->amount;
                        }

                //// for pi chart stripe paid invoices
                $paidInvAmount =DB::table('invoices')
                     ->select(DB::raw('sum(net_amount) as amount'),DB::raw('DATE_FORMAT(created_at, "%Y-%m-01") as month'))
                     ->groupBy('month')
                     ->orderBy('month','asc')
                     ->whereIn('status',['PAID-STRIPE','PAID-OFFLINE','PAID-BANKWIRE'])
                     ...
                     ->get();
                    //dd($paidInvAmount);
                  foreach($paidInvAmount as $paidInv){
                          $piChart[0] += $paidInv->amount;
                        }
              /// invoice net amount total where status Paid,Overdue,sent,deposit_paid          
            $revenues=DB::table('invoices')
                     ->select(DB::raw('sum(net_amount) as RevenueAmount'),DB::raw('DATE_FORMAT(created_at, "%Y-%m-01") as month'))
                     ->groupBy('month')
                     ->orderBy('month','asc')
                     ->where('status', '!=', 'DRAFT')
                     ->where('is_deleted','=',0)
                     ...
                     ->whereRaw('year(`created_at`) = ?', $yearData)
                      ...
                      foreach($revenues as $revenue){
                            $barChartData[$revenue->month]['revenue'] = $revenue->RevenueAmount;
                          }
              /// invoice deposit amount total where status DEPOSIT_PAID
              $invDepAm=DB::table('invoices')
                     ->select(DB::raw('sum(deposit_amount) as invoiceDepositAmount'),DB::raw('DATE_FORMAT(created_at, "%Y-%m-01") as month'))
                     ->groupBy('month')
                     ->orderBy('month','asc')
                     ...
                     ->whereRaw('year(`created_at`) = ?', $yearData)
                      ...
                      //dd($invDepAm);
                      foreach($invDepAm as $DepAmounts){
                            $barChartData[$DepAmounts->month]['invoiceDepositAmount'] = $DepAmounts->invoiceDepositAmount;
                          }


            $OverDueAmount=DB::table('invoices')
                     ->select(DB::raw('sum(net_amount) as OverDueAmount'),DB::raw('DATE_FORMAT(created_at, "%Y-%m-01") as month'))
                     ->groupBy('month')
                     ->orderBy('month','asc')
                     ...
                     ->whereRaw('year(`created_at`) = ?', $yearData)
                      ->get();
                      //dd($OverDueAmount);
                      foreach($OverDueAmount as $OvAmount){
                        $barChartData[$OvAmount->month]['OvAmount'] = $OvAmount->OverDueAmount;
                      }
            ...
            $finalBarData = [];
            $count = 0;
            foreach($barChartData as $key => $barData){
              $finalBarData[$count]['date'] = $key;
              $finalBarData[$count]['sent'] = isset($barData['sent']) ? $barData['sent'] : 0;
              $finalBarData[$count]['paid'] = isset($barData['paid']) ? $barData['paid'] : 0; /// stripe paid
              $finalBarData[$count]['OvAmount'] = isset($barData['OvAmount']) ? $barData['OvAmount'] : 0;
              $finalBarData[$count]['invoiceDepositAmount'] = isset($barData['invoiceDepositAmount']) ? $barData['invoiceDepositAmount'] : 0;
              $count++;
            }
            // print_r($finalBarData);exit;
            //$finalBarData = json_encode($finalBarData);
            $finalPiData = [];
           
            foreach($piChart as $key => $PiData){
              // $finalPiData[$key]['type'] = ($key == 0 ? "Paid" : ($key  == 1 ? "Deposit" : ($key  == 2 ? "Overdue" : "Sent")));
              $finalPiData[$key]['type'] = ($key == 0 ? "Paid" : ($key  == 1 ? "Deposit" : ($key  == 2 ? "Overdue" : ($key  == 3 ? "Sent" : "Cancel"))));
              $finalPiData[$key]['amount'] = $PiData;
            }
            //echo"<pre>";print_r($finalPiData);exit;

            /// Burst Graph Client Revenue Distribution
              $clientGet = Clients::where('user_id',$id)->latest()->get(); //dd($clientGet);
              $newChartData = [];
              $loop = 0;
              foreach ($clientGet as $clientData) {
                
                $clientInvoices = Invoice::select('invoice_number','net_amount','created_at')->where('user_id',$id)->where('client_id', $clientData->id)->where('status','!=','CANCEL')->get()->groupBy(function($val) {
                          return Carbon::parse($val->created_at)->format('Y-m-d');
                                            });
                foreach($clientInvoices as $key=>$clientInvoice){
                  $newChartData[$loop]['clientName'] = "$clientData->fname";
                  $newChartData[$loop]['date'][date("F", strtotime($key))] = $clientInvoice;
                }
                $loop++;
              }
             // dd($newChartData);

              $sentValue = $sentInvoiceAmount + $totalPaidAmount + $totalOfflineAmount + $totalBankWireAmount + $netOverdueAmount + $onlyInvoiceDepositAmount;

          return view('index',['invoices' =>$invoices,'clients' =>$clients, 'sentValue' => $sentValue,'finalPiData' =>$finalPiData,'newChartData' => $newChartData, 'requestYear' => $requestYear, 'clientRevenuYrs' => $clientRevenuYrs])->with('finalBarData', json_encode($finalBarData));
    }else{
